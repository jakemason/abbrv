...
namespace ImGui
{
    ...
    /// </summary>
    NOTIFY_INLINE VOID InsertNotification(const ImGuiToast& toast) { notifications.push_back(toast); }

    /// <summary>
    /// Remove a toast from the list by its index
    /// </summary>
    /// <param name="index">index of the toast to remove</param>
    NOTIFY_INLINE VOID RemoveNotification(int index) { notifications.erase(notifications.begin() + index); }
    ...
    NOTIFY_INLINE VOID RenderNotifications()
    {
        ...
        for (auto i = 0; i < notifications.size(); i++)
        {
            ...
            ImVec2 vp_pos = GetMainViewport()->Pos;

            ImGui::SetNextWindowPos(
                ImVec2(vp_pos.x + vp_size.x - NOTIFY_PADDING_X, vp_pos.y + vp_size.y - NOTIFY_PADDING_Y - height),
                ImGuiCond_Always,
                ImVec2(1.0f, 1.0f));
...
// pop warnings declared at the top of the file
#pragma warning(pop)
